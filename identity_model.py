...
class Identity_Model(AbstractModel.AbstractModel):
    ...
    def create_architecture(self, batch_size, lookup):
        ...
        #identity_matrix = ops.identity_initialization(self._embeddings_u, batch_size, self._no_senses, embedding_size)
        #print(identity_matrix.shape)
        #sense matrix. number of senses defined by variable. initialization = identity. For each
        #senseMatrix = tf.get_variable("S", initializer=identity_matrix)
        ...
        with tf.device("/cpu:0"):
            ...
            #use this as long as identity problem cannot be solve
            init = tf.constant_initializer(1.0)
            self._senseMatrix = tf.get_variable("s", shape=[batch_size, self._no_senses, embedding_size], validate_shape=False, initializer=init)
        ...
        self._specialized_modifier = self.select_senses(self._senseMatrix, self._selection_weights)

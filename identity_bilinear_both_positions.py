...
class Identity_Model_bilineaer_both_positions(AbstractModel.AbstractModel):

    def __init__(self, no_senses, vocab_size, dropout_bilinear_forms, dropout_matrix):
        super(Identity_Model_bilineaer_both_positions, self).__init__()
    ...
    def create_architecture(self, batch_size, lookup):
        ...
        with tf.device("/cpu:0"):
            ...
            self._matrix_lookup_mod = tf.get_variable("desicion_matrix_lookup_mod", initializer=gauss_matrix)
            self._matrix_lookup_head = tf.get_variable("desicion_matrix_lookup_head", initializer=gauss_matrix)
        ...
        self._D_mod = tf.nn.embedding_lookup(self._matrix_lookup_mod, self._u)
        self._D_head = tf.nn.embedding_lookup(self._matrix_lookup_head, self._v)
        s_mod = tf.nn.embedding_lookup(self._senseMatrix, self._u)
        s_head = tf.nn.embedding_lookup(self._senseMatrix, self._v)
        self._db = tf.get_variable("db", shape=[self._no_senses])

        self._selection_weights_mod = self.get_selection_weights(self._embeddings_u, self._embeddings_v, self._D_mod, self._db)
        self._selection_weights_head = self.get_selection_weights(self._embeddings_u, self._embeddings_v, self._D_head, self._db)

        print("selec weights")
        self._specialized_modifier = self.select_senses(s_mod, self._selection_weights_mod)
        self._specialized_modifier = super(Identity_Model_bilineaer_both_positions,
            self).l2_normalization_layer(
            self._specialized_modifier,
            1)

        self._specialized_head = self.select_senses(s_head, self._selection_weights_head)
        self._specialized_head = super(Identity_Model_bilineaer_both_positions,
                                           self).l2_normalization_layer(
            self._specialized_head,
            1)

        self._architecture = self.compose(u=self._specialized_modifier, v=self._specialized_head,
                                          W=self._W, b=self._b, bilinear_forms=self._bilinear_forms)


        self._architecture_normalized = super(
            Identity_Model_bilineaer_both_positions,
            self).l2_normalization_layer(
            self._architecture,
            1)
    ...
    @property
    def selection_weights_mod(self):
        return self._selection_weights_mod

    @property
    def selection_weights_head(self):
        return self._selection_weights_head
